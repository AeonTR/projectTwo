Shader "KriptoFX/AE/Particle" {
	Properties {
		...
		[Header(Fading)] [Space] [Toggle(USE_SOFT_PARTICLES)] _UseSoft ("Use Soft Particles", Float) = 0
		_InvFade ("Soft Particles Factor", Float) = 1
		[Toggle(USE_SOFT_PARTICLES_INVERTED)] _UseSoftInverted ("Use Inverted Soft Particles", Float) = 0
		...
		[Space] [Header(Rendering)] [Toggle(USE_FRAME_BLENDING)] _UseFrameBlending ("Use Frame Blending", Float) = 0
		_IsDecalProjector ("Is Decal Projector", Float) = 0
