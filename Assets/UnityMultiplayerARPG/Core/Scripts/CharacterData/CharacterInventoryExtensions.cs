...
namespace MultiplayerARPG
{
    public static class CharacterInventoryExtensions
    {
        ...
        public static bool EnhanceUseEnchantingPaper(this IPlayerCharacterData character, InventoryType inventoryType, int index, int paperId, out UITextKeys gameMessage)
        {
#if UNITY_EDITOR || UNITY_SERVER
            switch (inventoryType)
            {
                case InventoryType.NonEquipItems:
                    return BaseItem.GenerateRandomAttrNonEquipItem(character, index, paperId, out gameMessage);
                case InventoryType.EquipItems:
                    return BaseItem.GenerateRandomAttrEquipItem(character, index, paperId, out gameMessage);
                case InventoryType.EquipWeaponRight:
                    return BaseItem.GenerateRandomAttrRightHandItem(character, paperId, out gameMessage);
                case InventoryType.EquipWeaponLeft:
                    return BaseItem.GenerateRandomAttrLeftHandItem(character, paperId, out gameMessage);
            }
            gameMessage = UITextKeys.UI_ERROR_INVALID_ITEM_DATA;
            return false;
#else
        ...
        public static bool EnhanceUseEnchantingCost(this IPlayerCharacterData character, InventoryType inventoryType, int index, int costId, out UITextKeys gameMessage)
        {
#if UNITY_EDITOR || UNITY_SERVER
            switch (inventoryType)
            {
                case InventoryType.NonEquipItems:
                    return BaseItem.ChangeRandomAttrNonEquipItem(character, index, costId, out gameMessage);
                case InventoryType.EquipItems:
                    return BaseItem.ChangeRandomAttrEquipItem(character, index, costId, out gameMessage);
                case InventoryType.EquipWeaponRight:
                    return BaseItem.ChangeRandomAttrRightHandItem(character, costId, out gameMessage);
                case InventoryType.EquipWeaponLeft:
                    return BaseItem.ChangeRandomAttrLeftHandItem(character, costId, out gameMessage);
            }
            gameMessage = UITextKeys.UI_ERROR_INVALID_ITEM_DATA;
            return false;
#else
