...
namespace MultiplayerARPG
{
    public partial class DefaultServerInventoryMessageHandlers : MonoBehaviour, IServerInventoryMessageHandlers
    {
        ...
        public async UniTaskVoid HandleRequestUseEnchantingPaper(
            RequestHandlerData requestHandler, 
            RequestUseEnchantingPaperMessage request, 
            RequestProceedResultDelegate<ResponseUseEnchantingPaperMessage> result)
        {
            await UniTask.Yield();

            if (!GameInstance.ServerUserHandlers.TryGetPlayerCharacter(requestHandler.ConnectionId, out IPlayerCharacterData playerCharacter))
            {
                result.InvokeError(new ResponseUseEnchantingPaperMessage()
                {
                    message = UITextKeys.UI_ERROR_NOT_LOGGED_IN,
                });
                return;
            ...
            if (!playerCharacter.EnhanceUseEnchantingPaper(request.inventoryType, request.index, request.paperId, out UITextKeys gameMessage))
            {
                result.InvokeError(new ResponseUseEnchantingPaperMessage()
                {
                    message = gameMessage,
            ...
            result.InvokeSuccess(new ResponseUseEnchantingPaperMessage()
            {
                message = gameMessage,
            });
        }

        public async UniTaskVoid HandleRequestUseEnchantingCost(
            RequestHandlerData requestHandler,
            RequestUseEnchantingCostMessage request,
            RequestProceedResultDelegate<ResponseUseEnchantingCostMessage> result)
        {
            await UniTask.Yield();

            if (!GameInstance.ServerUserHandlers.TryGetPlayerCharacter(requestHandler.ConnectionId, out IPlayerCharacterData playerCharacter))
            {
                result.InvokeError(new ResponseUseEnchantingCostMessage()
                {
                    message = UITextKeys.UI_ERROR_NOT_LOGGED_IN,
                });
                return;
            ...
            if (!playerCharacter.EnhanceUseEnchantingCost(request.inventoryType, request.index, request.costId, out UITextKeys gameMessage))
            {
                result.InvokeError(new ResponseUseEnchantingCostMessage()
                {
                    message = gameMessage,
            ...
            result.InvokeSuccess(new ResponseUseEnchantingCostMessage()
            {
                message = gameMessage,
            });
        }
