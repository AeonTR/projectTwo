    ...
    public partial class SQLiteDatabase
    {
        ...
        private bool ReadStorageItem(SqliteDataReader reader, out CharacterItem result)
        {
            if (reader.Read())
            {
                ...
                result.ReadEnchantingDatas(reader.GetString(12));
                return true;
            }
        ...
        public override UniTask<List<CharacterItem>> ReadStorageItems(StorageType storageType, string storageOwnerId)
        {
            ...
            }, "SELECT id, dataId, level, amount, durability, exp, lockRemainsDuration, expireTime, randomSeed, ammo, sockets, version, enchantings FROM storageitem WHERE storageType=@storageType AND storageOwnerId=@storageOwnerId ORDER BY idx ASC",
                new SqliteParameter("@storageType", (byte)storageType),
                new SqliteParameter("@storageOwnerId", storageOwnerId));
